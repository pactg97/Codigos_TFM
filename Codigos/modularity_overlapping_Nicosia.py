# -*- coding: utf-8 -*-
"""
Created on Fri Apr 30 12:07:32 2021
...
#                    l4[i,j,s]=m.addVar(vtype=GRB.CONTINUOUS,lb=0,ub=1,name="l4"+"_"+str(i)+"_"+str(j)+"_"+str(s))
        ...
        # Función objetivo
        m.setObjective(sum([sum([sum([((u[i,s]+u[j,s])/2*W[i,j]-sum([(u[k,s]+u[j,s])/2 for k in range(1,n+1)])*sum([(u[i,s]+u[k,s])/2 for k in range(1,n+1)])*sum([W[i,k] for k in range(1,n+1)])*sum([W[j,k] for k in range(1,n+1)])/(W_total*n*n)) for j in range(1,n+1)]) for i in range(1,n+1)]) for s in range(1,nc+1)]), GRB.MAXIMIZE)
...
#        else:
#            W1[i,j]=0
modularity_overlapping(W,nodos,0.3,3)
